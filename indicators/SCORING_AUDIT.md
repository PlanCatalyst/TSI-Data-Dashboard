...
| 27 | popdens   | ctx     | ctxmisc  | EN.POP.DNST | ? | see note | review | **VERIFIED 2026-06-28: data now flows end-to-end (215/216 countries non-null), BUT `DensityScorer` `(value/0.7)*100` saturates — 99.6% of source rows clamp to pipeline 100 / published 0, leaving only ~5 distinct published values. Effectively a constant.** Recommend adopting the banded formula already documented in [indicators.yaml](indicators.yaml) (>=250 -> 100, >=100 -> 75, >=75 -> 50, >=25 -> 25, else 0), which yields a real 5-way spread (624/800/398/1110/707 over 3,639 source rows). Open semantic question: population density has no universal good/bad direction — confirm with PlanCatalyst whether it should be a scored+inverted indicator at all, or context/display-only. Decision owner: **Anthony**; semantic sign-off via **Thomas → PlanCatalyst**. |
...
- **Pop density scorer mismatch** (1, still open): `popdens` — **end-to-end run on 2026-06-28 confirmed the `series_code` fix works (`EN.POP.DNST` flows clean -> score -> publish, 215/216 countries non-null), but empirically confirmed the scorer is broken: `DensityScorer = (value/0.7)*100` clamps 99.6% of rows to the bound, producing a near-constant published 0 (~5 distinct values).** Canonical fix = implement the banded 0/25/50/75/100 formula from [indicators.yaml](indicators.yaml). Pending PlanCatalyst decision on whether density should be scored/inverted or display-only.

Net: **28 of 28 indicators flow end-to-end through scoring and into published JSON as of 2026-06-28** (gii, mpi, ndgain composite, state live 2026-05-17; `hdi` live 2026-06-01 as the `pri` proxy replacing `conces`; `popdens` now emits a `series_code` and reaches publish). 1 indicator scores but with a **broken (saturating) formula**:
- `popdens` — the World Bank cleaner now emits `series_code = EN.POP.DNST` (fixed 622e3bf), so rows are scored and published (215/216 countries non-null). But `DensityScorer = (value/0.7)*100` saturates to a near-constant published 0. Fix = adopt the banded formula (see direction table row 27 and the gap bullet above). Owner: **Anthony**; semantic sign-off via **Thomas → PlanCatalyst**.
